...
router.get("/stats", (req, res) => {
  ...
  db.get("SELECT COUNT(*) as total FROM medications WHERE user_id = ?", [req.user.id], (err, totalMeds) => {
    if (err) {
      ...
      (err, takenToday) => {
        ...
        db.all(
          ...
          (err, dailyLogs) => {
            ...
            const adherenceRate = totalMeds.total > 0 ? Math.round((takenToday.taken / totalMeds.total) * 100) : 0
            ...
            const calculateStreak = async () => {
              ...
              const checkDate = (date) => {
                return new Promise((resolve) => {
                  ...
                  db.get(
                    ...
                    (err, result) => {
                      if (err || !result || result.total === 0 || result.taken < result.total) {
                        resolve(false)
                      } else {
                        resolve(true)
                      }
              ...
              while (checking && streakCount < 365) {
            ...
            calculateStreak()
              .then((streakCount) => {
                res.json({
                  totalMedications: totalMeds.total || 0,
                  takenToday: takenToday.taken || 0,
